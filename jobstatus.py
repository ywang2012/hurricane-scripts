...
def parseArgv(argv) :
  ...
  if ( len(args) >=  1 ) :
    ...
    if oneargs:
      ...
      argvs.append('%4d%02d%02d'%(curryear,currmon,currday))
    else :
    ...
    argvs.append('%4d%02d%02d'%(curryear,currmon,currday))
    #if currh < 18 :
    #  argvs.append(['gfs00Z','ekf00Z'])
    #else :
    #  argvs.append(['gfs12Z','ekf12Z'])
    argvs.append(False)
...
def main(datein,casein=False) :
  ...
  cmdlist = ["/home/kwthomas/wat/watcher %s" % datein]

  checker=subprocess.Popen(cmdlist,shell=True,
  ...
  if not casein :
    ...
    if isinstance(casein,list) :
      casenames = []
      for case in casein :
        casemth = re.match('(gfs|ekf)(\d{2})Z',case)
        if casemth :
  ...
  for case in casenames :
      ...
      if casemth :
        print "%s %s" % (cprint(case,'cyan'),casemth.group(1))
